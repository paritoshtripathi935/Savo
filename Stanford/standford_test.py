...
def Data():
    for i in range(0, page):
        ...
        soup = BeautifulSoup(driver.page_source, 'lxml')  # this is used to find the things we need in the source code

        link_class = soup.findAll(class_='node node--type-course')  # titles of youtube videos is stored here

        for names in link_class:
            name_tag = names.find("h3")
